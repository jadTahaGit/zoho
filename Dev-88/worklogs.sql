...
LEFT JOIN "Config: Mitarbeiter" AS  employee ON worklogs."Username"  = employee."Mitarbeiter" -- correct
...
JOIN "JiraCloudTempoWorklogs" AS  TEMPO_WORKLOGS ON TEMPO_WORKLOGS."JiraWorklogId"  = CLOUD_WORKLOGS."Worklog ID" 
JOIN "Software Issues (Jira Software Cloud)" AS  CLOUD_ISSUES ON CLOUD_WORKLOGS."Issue ID"  = CLOUD_ISSUES."Issue ID" 
JOIN "Config: Mitarbeiter" AS  WORKER ON WORKER."JIRA_Cloud_UserID"  = TEMPO_WORKLOGS."AuthorId" -- correct
LEFT JOIN "Software Projects (Jira Software Cloud)" AS  CLOUD_PROJECTS ON CLOUD_PROJECTS."Project ID"  = CLOUD_ISSUES."Project ID" 

SELECT
		 ...
		 ELT(GREATEST(MAX(FIELD(targetTable."Lead Status (Companies)", 'NEW', 'INACTIVE', 'BAD_TIMING', 'ATTEMPTED_TO_CONTACT', 'CONNECTED', 'ACTIVE', 'LOST', 'UNQUALIFIED')), MAX(FIELD(targetTable."Lead Status (Contacts)", 'NEW', 'INACTIVE', 'BAD_TIMING', 'ATTEMPTED_TO_CONTACT', 'CONNECTED', 'ACTIVE', 'LOST', 'UNQUALIFIED'))), 'NEW', 'INACTIVE', 'BAD_TIMING', 'ATTEMPTED_TO_CONTACT', 'CONNECTED', 'ACTIVE', 'LOST', 'UNQUALIFIED') AS "Lead Status",
		 ELT(GREATEST(MAX(FIELD(targetTable."Lifecycle Stage (Companies)", '', 'evangelist', 'other', 'lead', 'marketingqualifiedlead', 'salesqualifiedlead', 'opportunity', 'customer', 'subscriber')), MAX(FIELD(targetTable."Lifecycle Stage (Contacts)", '', 'evangelist', 'other', 'lead', 'marketingqualifiedlead', 'salesqualifiedlead', 'opportunity', 'customer', 'subscriber'))), 'other', 'other', 'other', 'lead', 'marketingqualifiedlead', 'salesqualifiedlead', 'opportunity', 'customer', 'subscriber') AS "Lifecycle Stage",
		 MAX(targetTable."Target Account (Companies)") AS "Target Account ",
		 MAX(targetTable."ICP (Companies)") AS "ICP",
		 LEAST(MAX(targetTable."Became Sub (Companies)"), MAX(targetTable."Became Sub (Contacts)")) AS "Became SUB",
		 LEAST(MAX(targetTable."Became LEAD (Companies)"), MAX(targetTable."Became LEAD (Contacts)")) AS "Became LEAD",
		 LEAST(MAX(targetTable."Became MQL (Companies)"), MAX(targetTable."Became MQL (Contacts)")) AS "Became MQL",
		 LEAST(MAX(targetTable."Became SQL (Companies)"), MAX(targetTable."Became SQL (Contacts)")) AS "Became SQL",
		 LEAST(MAX(targetTable."Became OPP (Companies)"), MAX(targetTable."Became OPP (Contacts)")) AS "Became OPP",
		 LEAST(MAX(targetTable."Became CUS (Companies)"), MAX(targetTable."Became CUS (Contacts)")) AS "Became CUS",
...
JOIN "Companies (HubSpot CRM)" AS  companies ON contacts."Primary Associated Company ID"  = companies."Company ID"  
) AS  targetTable 
GROUP BY targetTable."Date",
	  targetTable."Company ID (Companies)" 
